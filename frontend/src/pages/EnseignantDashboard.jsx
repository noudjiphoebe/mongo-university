...
const EnseignantDashboard = () => {
  ...
  useEffect(() => {
    const fetchEmploiDuTemps = async () => {
      try {
        const response = await axios.get('http://localhost:3001/api/emploi-du-temps/enseignant');
        setEmploiDuTemps(response.data);
      } catch (error) {
        console.error('Erreur chargement emploi du temps:', error);
        // En attendant l'API, utilisez des données mock
        setEmploiDuTemps([
          { id: 1, cours: 'Mathématiques', jour: 'Lundi', heure: '08:00-10:00', salle: 'A101' },
          { id: 2, cours: 'Physique', jour: 'Mardi', heure: '10:00-12:00', salle: 'B202' }
        ]);
      } finally {
        setLoading(false);
      }
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold mb-6">Espace Enseignant</h1>
